(function () {
  ...
  function latinWordCount(text) {
    const stop = new Set(['of', 'in', 'on', 'at', 'to', 'for', 'with', 'by', 'the', 'and', 'or', 'a', 'an']);
    return (String(text).match(/\b[a-zA-Z]{3,}\b/g) || []).filter((w) => !stop.has(w.toLowerCase())).length;
  }
  ...
  function needsTranslation(text) {
    return latinWordCount(normalizeText(text)) >= 2;
  }
  ...
  function translateTitle(text) {
    if (!text) return '';
    const norm = normalizeText(text);
    if (!needsTranslation(norm)) return norm;

    const exact = findExact(norm);
    if (exact) return exact;

    const partial = applyPhrases(norm.replace(/\[PDF[^\]]*\]/gi, '').trim());
    if (cjkCount(partial) >= 4 && latinWordCount(partial) < latinWordCount(norm)) {
      return partial + extractPdfSuffix(norm);
    }

    return `日本央行：${classifyFallback(norm)}${extractPdfSuffix(norm)}`;
  }
  ...
  function translateSpeaker(text) {
    ...
    const exact = findExact(norm);
    if (exact) return exact;
    const partial = applyPhrases(norm);
    return cjkCount(partial) >= 2 ? partial : norm;
  }
