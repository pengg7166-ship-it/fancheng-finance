...
function renderPolicyReadingList(items) {
  ...
  return `<div class="policy-reading-list">${items.map((item, i) => renderPolicyReadingCard(item, i + 1)).join('')}</div>`;
}
...
function refreshPolicyPanelSections(panel, source) {
  ...
  if (scroll) {
    const listItems = getFilteredPolicyItemsForView(fullSource.items);
    scroll.innerHTML =
...
function renderGeopoliticsReadingList(items) {
  ...
  return `<div class="policy-reading-list geo-reading-list">${items.map((item, i) => renderGeopoliticsReadingCard(item, i + 1)).join('')}</div>`;
}
...
function renderPolicyPanel(source) {
  ...
  const filtered = getFilteredPolicyItemsForView(source.items);
...
function applyStartupData(data, { fromCache = false } = {}) {
...
function applyPolicyLiveData(source) {
  ...
  refreshPolicyPanelSections(panel, source);
...
function applyGeopoliticsLiveData(source) {
  ...
  if (panel) refreshGeopoliticsPanelSections(panel, source);
...
function setupGeopoliticsPanel() {
  ...
  panel.addEventListener('click', (e) => {
    ...
    if (regionBtn) {
      ...
      refreshGeopoliticsPanelSections(panel);
      return;
    }
...
async function fetchDataInBackground(force = false) {
  try {
    ...
    applyStartupDataSafe(data, { fromCache: data.fromCache });
    showErrors(data.errors || []);
    indicesLoading = false;
