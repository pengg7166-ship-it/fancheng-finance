 ...
 * 化工系区间宽度网格校准 — 目标 OOS range coverage ≥ 75%

 * 训练 2019-2022 · 评估 2023-2026

 *

 * FANCHENG_DATA_DRIVE=E NODE_OPTIONS=--max-old-space-size=4096 node scripts/calibrate-chemical-range.js --id fg
...
const TARGET_IDS =
  ...
  CLI_IDS.length > 0
    ...
    ? CLI_IDS.filter((id) => chemicalCal.CALIBRATED_IDS.includes(id))

    : [...chemicalCal.CALIBRATED_IDS];
...
function maeHighLow(samples, cal, from, to) {
  ...
  for (const s of samples) {
    ...
    const band = chemicalCal.computeChemicalRangeBand({
...
function resolveCrossSource(id) {
  ...
  const cal = chemicalCal.DEFAULT_PARAMS[id];

  if (!cal) return null;

  if (cal.taCrossBias) return 'ta';

  if (cal.lCrossBias) return 'l';

  if (cal.egCrossBias) return 'eg';
...
function precomputeSamples(id, bars, crossBars = null, crossId = null) {
  ...
  for (let i = 20; i < bars.length - 1; i += 1) {
    ...
    const volWindow = chemicalCal.DEFAULT_PARAMS[id]?.volWindow || 20;
...
function evalBaseline(samples, id, from, to) {
  ...
  const cal = { ...chemicalCal.DEFAULT_PARAMS[id], upMult: 1.0, downMult: 1.0, _id: id };
...
function gridSearch(id, samples) {
  ...
  const downRange = [];
  const lowBiasRange = id === 'fg' ? [0.008, 0.01, 0.012, 0.015, 0.018] : [0, 0.005, 0.01, 0.015, 0.02];
  ...
  for (const upMult of upRange) {
    ...
    for (const downMult of downRange) {
      ...
      for (const lowBiasPct of lowBiasRange) {
        ...
        const cal = { ...chemicalCal.DEFAULT_PARAMS[id], upMult, downMult, lowBiasPct, _id: id };
        ...
        const mae = maeHighLowShared(
          ...
          (s) =>
            ...
            chemicalCal.computeChemicalRangeBand({
...
function mergeExistingPayload() {
  ...
  const fp = chemicalCal.calFilePath();
...
function main() {
  ...
  const payload = {
    ...
    version: chemicalCal.CAL_VERSION,
    ...
    ...Object.fromEntries(
      ...
      chemicalCal.CALIBRATED_IDS.map((id) => [

        id,

        results[id]?.cal || existing[id] || chemicalCal.DEFAULT_PARAMS[id],
  ...
  const fp = chemicalCal.saveCalParams(payload);
