/**
 * 黑色系区间宽度网格校准 — 目标 OOS range coverage ≥ 75%
 * 训练 2019-2022 · 评估 2023-2026
 *
 * FANCHENG_DATA_DRIVE=E NODE_OPTIONS=--max-old-space-size=4096 node scripts/calibrate-black-range.js --id rb
 */
const fs = require('fs');
...
const TARGET_IDS =
  CLI_IDS.length > 0
    ? CLI_IDS.filter((id) => blackCal.CALIBRATED_IDS.includes(id))
    : [...blackCal.CALIBRATED_IDS];
...
function precomputeSamples(id, bars, crossBars = null, crossId = null) {
  ...
  for (let i = 20; i < bars.length - 1; i += 1) {
    ...
    const volWindow = blackCal.DEFAULT_PARAMS[id]?.volWindow || 20;
...
function evalBaseline(samples, id, from, to) {
  const cal = { ...blackCal.DEFAULT_PARAMS[id], upMult: 1.0, downMult: 1.0, _id: id };
...
function gridSearch(id, samples) {
  ...
  for (const upMult of upRange) {
    for (const downMult of downRange) {
      const cal = { ...blackCal.DEFAULT_PARAMS[id], upMult, downMult, _id: id };
      ...
      const mae = maeHighLowShared(
        samples,
        (s) =>
          blackCal.computeBlackRangeBand({
            instrumentId: cal._id,
            baselineClose: s.baselineClose,
            upExtents: s.upExtents,
            downExtents: s.downExtents,
...
function main() {
  ...
  const payload = {
    ...
    ...Object.fromEntries(
      blackCal.CALIBRATED_IDS.map((id) => [id, results[id]?.cal || existing[id] || blackCal.DEFAULT_PARAMS[id]]),
    ),
    metrics: {
      ...(existing.metrics || {}),
      ...Object.fromEntries(
  ...
  const fp = blackCal.saveCalParams(payload);
  console.log('\nSaved', fp);
