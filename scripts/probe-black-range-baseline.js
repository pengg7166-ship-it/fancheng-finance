/**
 * 黑色系 baseline OOS range coverage 探针（校准前/后对比）
 * FANCHENG_DATA_DRIVE=E NODE_OPTIONS=--max-old-space-size=4096 node scripts/probe-black-range-baseline.js
 */
const fs = require('fs');
...
const BLACK_IDS = [...blackCal.CALIBRATED_IDS];
const REGRESSION_IDS = ['au', 'cu'];
...
function probeInstrument(id) {
  ...
  return {
    id,
    calibrated: blackCal.isCalibrated(id) || nonferrousCal.isCalibrated(id) || id === 'au' || id === 'ag',
    scored,
...
function main() {
  const ids = [...BLACK_IDS, ...REGRESSION_IDS];
  const results = ids.map((id) => probeInstrument(id));

  const payload = {
    version: 'probe-black-range-v1',
    testFrom: TEST_FROM,
    testTo: TEST_TO,
    instruments: results,
    summary: {
      black: results.filter((r) => BLACK_IDS.includes(r.id)),
      regression: results.filter((r) => REGRESSION_IDS.includes(r.id)),
    },
  };

  const outJson = path.join(__dirname, '..', '_probe-black-range-out.json');
  const outTxt = path.join(__dirname, '..', '_probe-black-range-out.txt');
  fs.writeFileSync(outJson, JSON.stringify(payload, null, 2));

  const lines = [
    '=== Black Range Calibration Probe ===',
