/**
 * 农产品系 baseline OOS range coverage 探针（校准前/后对比）
 * FANCHENG_DATA_DRIVE=E NODE_OPTIONS=--max-old-space-size=4096 node scripts/probe-agricultural-range-baseline.js
 */
const fs = require('fs');
...
const AGRI_IDS = [...agriCal.CALIBRATED_IDS];
const REGRESSION_IDS = ['au', 'cu', 'rb', 'sc'];
...
function predMethod(id) {
  ...
  if (agriCal.isCalibrated(id)) return 'agricultural-range-calibrated';
  return 'default';
}
...
function main() {
  const ids = [...AGRI_IDS, ...REGRESSION_IDS];
  const results = ids.map((id) => probeInstrument(id));

  const payload = {
    version: 'probe-agricultural-range-v1',
    testFrom: TEST_FROM,
    testTo: TEST_TO,
    instruments: results,
    summary: {
      agricultural: results.filter((r) => AGRI_IDS.includes(r.id)),
      regression: results.filter((r) => REGRESSION_IDS.includes(r.id)),
    },
  };

  const outJson = path.join(__dirname, '..', '_probe-agricultural-range-out.json');
  const outTxt = path.join(__dirname, '..', '_probe-agricultural-range-out.txt');
  fs.writeFileSync(outJson, JSON.stringify(payload, null, 2));

  const lines = [
    '=== Agricultural Range Calibration Probe ===',
